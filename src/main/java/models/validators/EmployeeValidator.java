...
/**
 *
 * 従業員インスタンスに設定されている値のバリディーションを行うクラス
 *
 */
public class EmployeeValidator {

    /**
     * 従業員インスタンスの各項目についてバリディーションを行う
     ...
     *@param code 社員番号
     *@param codeDuplicateCheckFlag社員番号の重複チェックを実施するかどうか（実施する:true 実施いない:false)
     *@return エラーメッセージ
     */
    private static String validateCode(EmployeeService service, String code, Boolean codeDuplicateCheckFlag) {
        ...
        if(codeDuplicateCheckFlag) {
            ...
            //同一社員番号がすでに登録されてる場合はエラーメッセージを返却
            if(employeeCount > 0) {
